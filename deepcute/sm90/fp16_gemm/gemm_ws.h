...
struct GemmFp16SM90 {
    ...
    struct alignas(128) SharedStorage {
        ...
        array<Dtype, size(SmemLayoutA{})> A;
        array<Dtype, size(SmemLayoutB{})> B;
        array<Dtype, size(SmemLayoutC{})> C;
    ...
    void operator()(G2STmaCopyA const& tma_a,
        ...
        if (warp_id == 0 && elect_one_sync()) { // the same as tid == 0
        ...
        if (warpgroup_id == 0) {
            // seems if don't use this reg trick still work  
    ...
    void consumer(SharedStorage &smem,
        ...
        Tensor gC_tma = local_tile(C, CTATile{}, make_coord(_, _, _), Step<_1, _1, X>{}); // (cta_m, cta_n, m, n)
        Tensor gC = make_tensor(make_gmem_ptr<AccType>(nullptr), make_layout(take<0, 2>(CTATile{}), LayoutRight{}));
        ...
        Tensor t_rC = thr_mma.partition_fragment_C(gC); // (CPY, rest_m, rest_n), can't partition a tma tensor
        // Tensor t_rC = partition_fragment_C(tiled_mma, take<0, 2>(CTATile{})); // but this would work
